#' @title Bayesian inference for a spatial binary functional linear mixed model (SBFLMM)
#'
#' @description 
#' Performs Bayesian inference for a spatial binary mixed model, 
...
#'
#' @param initial A (p+G+3)-dimsional vector giving the initial values for the parameters 
...
#'
#' @param nbd_list A list of length G, where each element contains the neighborhood structure (of class \code{nb}) for a corresponding group.
#' 
...
#' }
#' @seealso
#' \code{\link{posterior_samples_bflm}}
#' \code{\link{posterior_samples_sbflm}}
#' \code{\link{posterior_samples_bflmm}}
#' 
#' @examples
#' 
...
#' res <- posterior_samples_sbflmm(M=8000, initial=initial_values, 
...
#' 
#' @export
posterior_samples_sbflmm <- function(M, initial, proposal_var_fixed, proposal_var_sigma2, proposal_var_rand, autotune, 
  ...
  res <- rw_dd(M, initial, 
               ...
               autotune,
               X_list, data_list, nbd_list, p, basis, M_auxz, length(data_list), prior_a, prior_b)
